    ...
    return class CMakeBuildProvider extends EventEmitter {
        ...
        createVisualStudioTarget(target_name) {
            return {
                ...
                cwd : this.source_dir,
                args : [this.custom_args, '--build', this.build_dir, '--target',target_name,'--','/maxcpucount','/clp:NoSummary;ErrorsOnly;Verbosity=quiet'],
        ...
        createMakeFileTarget(target_name) {
            return {
                ...
                exec : this.executable,
                cwd : this.source_dir,
                args : [this.custom_args, '--build', this.build_dir, '--target',target_name,'--','-j'+os.cpus().length],
                errorMatch : compileErrorMatch.concat(generateErrorMatch),
                sh : false
            };
        }
